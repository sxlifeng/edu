...
$(document).keydown(function (e) { 
 ...
 function logout(b) {
	$('#sessionInfoDiv').html('');
	$.post('admin.php/Index/loginout');
	top.location = "admin.php";
}
function changepsw() {
	$('#changepswForm').form('clear');
	$('#changepswDlg').dialog('open').dialog('setTitle', '修改密码');

}


function changepsw_ok() {
	$.messager.defaults = {
		ok : "确定",
		cancel : "取消"
	};
		...
		if (r) {
			if ($("#newpassword1").val()==$("#newpassword2").val()) {
				$('#changepswForm').form('submit', {
					url : "admin.php/User/editPwd",
					dataType : 'json'
					
					
				});
			}else {
				$.messager.alert('操作提示', '新密码不一致', 'error');
				return false;
			}
			$('#changepswDlg').dialog('close');
		}
	});
}